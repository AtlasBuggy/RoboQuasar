...
class RoboQuasarPlotter:
    def __init__(self, animate, enable_plotting, enable_kalman, course_map, inner_map, outer_map, key_press_fn,
        ...
        self.corrected_gps_plot = RobotPlot("corrected gps", color="fuchsia", enabled=True)
        self.map_plot = RobotPlot("map", color="purple", marker='.', put_in_legend=False)
        self.inner_map_plot = RobotPlot("inner map", enabled=True, put_in_legend=False)
        self.outer_map_plot = RobotPlot("outer map", enabled=True, put_in_legend=False)

        # angle based plots
        self.compass_plot = RobotPlot("quasar filtered heading", color="purple", put_in_legend=False)
        self.compass_plot_imu_only = RobotPlot("imu filtered heading", color="magenta", put_in_legend=False)
        self.sticky_compass_plot = RobotPlot("sticky compass", color="gray", enabled=False, put_in_legend=False)
        self.steering_plot = RobotPlot("steering angle", color="gray", enabled=False, put_in_legend=False)

        # indicator dots and lines
        self.checkpoint_plot = RobotPlot("checkpoint", color="green", marker='.', linestyle='', markersize=8, put_in_legend=False)
        self.goal_plot = RobotPlot("checkpoint goal", color="cyan", put_in_legend=False)
        self.recorded_goal_plot = RobotPlot("recorded checkpoint goal", "blue", put_in_legend=False)
        self.current_pos_dot = RobotPlot("current pos dot", "blue", marker='.', markersize=8, put_in_legend=False)

        # kalman plots
        self.enable_kalman = enable_kalman
        self.kalman_recomputed_plot = RobotPlot("kalman recomputed", color="lawngreen", enabled=self.enable_kalman)
        self.kalman_recorded_plot = RobotPlot("kalman recorded", color="gray", enabled=self.enable_kalman)

        self.steering_angle_plot = RobotPlot("steering angle", put_in_legend=False)
        ...
        if animate:
            self.plotter = LivePlotter(
                ...
                matplotlib_events=dict(key_press_event=key_press_fn), enable_legend=True,
...
video_sets = {
    ...
    "buggy videos": (
        ("Ascension 10-17 roll 3-2", "/Users/Woz4tetra/Google Drive/Atlas Docs/Project Files/videos/buggy_videos/", "mp4"),
        ("Icarus 10-11 roll 5 (+hill 1)", "/Users/Woz4tetra/Google Drive/Atlas Docs/Project Files/videos/buggy_videos/", "mp4"),
    ),
}
